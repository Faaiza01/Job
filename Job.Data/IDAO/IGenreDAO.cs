...
namespace Job.Data.IDAO
{
    ...
    {
        IList<Employer> GetGenres(JobContext context);
        ...
        List<AppliedJobsList> SearchByJobTitle(JobContext context, string text, string UserId);
            // Genre GetGenre(JobContext context, int id);
            //IList<Music> GetMusics(JobContext context, int id);
            //void AddMusicToCollection(JobContext context, Genre genre, Music music);

        //int GetGenreByMusicID(JobContext context, int musicId);
